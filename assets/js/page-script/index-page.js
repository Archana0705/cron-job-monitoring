    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success == 1) {
                ...
                studentTableId = data.sspstudentid;
                data.aadhaar_no = data.aadhaar_no.slice(0, 2) + '******' + data.aadhaar_no.slice(8)
                    ...
                    "columns": [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                if (data) {
                                    return data +
                                        '<input type="hidden" id="studentschemeid" value="' + row.studentschemeid + '">' +
                                        '<input type="hidden" id="scheme_rule_id" value="' + row.scheme_rule_id + '">' +
                                        '<input type="hidden" id="selected_scheme_name" value="' + row.scheme_name + '">';
                                } else {
                                    return '-';
                                }
                            ...
                            render: function (data, type, row) {
                                ...
                                if (data === true) {
                                    return '<div class="action-buttons">' +
                                        '<a class="btn add-btn view-scheme info" href="#" data-scheme-id="' + schemeid + '">' +
                                        '<i class="bi bi-eye"></i> View</a>' +
                                        '<a class="btn btn-success btn-sm applied_application" href="#"  data-filepath="' + row.filepath + '">' +
        ...
        error: function (xhr, status, error) {
            ...
            $(document).on('click', '.apply_link', function (event) {
                ...
                $.ajax({
                    ...
                    headers: { "X-APP-KEY": "te$t", "X-APP-NAME": "te$t" },
                    data: {
                      studentid: studentTableId || 0,
                      subschemeid: studsub_Id || 0,
                    },
                    success: function (response) {
                        $.ajax({
                                    ...
                                    $.ajax({
                                        ...
                                        complete: function () {
                                            ...
                                            if (data.scheme_category == 'Exclusive') {
                                                ...
                                                $('#exclusiveModalMsg').text('The Below Schemes are already applied: ');
                                                $('#exclusiveModalLabel').css('color', 'red');
                                                $('#exclusiveModalLabel').text('You are not eligible for this scheme');
                                                $('#exclusiveModal').modal('show');
                                            }
                                            else if (data.scheme_category == "Exclusive With Exceptions") {
                                                $('.schemesList').text('');
                                                exclusiveSchemes = JSON.parse(data.exclusiondetail);


                                                var filteredSchemes = data_scheme.filter(function (scheme) {
                                                    return scheme.is_applied === true && !exclusiveSchemes.some(function (exception) {
                                ...
                                $(document).ready(function () {
                                    ...
                                    inputs.each(function (index1) {
                                        $(this).on("keyup", function (e) {
                                            ...
                                            if (!$(inputs[inputsNo - 1]).is(":disabled") && $(inputs[inputsNo - 1]).val() !== "") {
                                ...
                                $.ajax({
                                    ...
                                    complete: function () {
                                        ...
                                        if (data.scheme_category == 'Exclusive') {
                                            ...
                                            // $('#exclusiveModalfalse').modal('show');
                                            $('#form-scheme-exceptions').text('If you apply for this scheme, you will not be eligible for any other schemes.');
                                        }
                                        else if (data.exclusiondetail !== null) {
                                            const jsonString = data.exclusiondetail;
                                            $('#form-scheme-exceptions').text('');
                                            ...
                                            $('#form-scheme-exceptions').text('');
                                            $('#form-scheme-exceptions').text('In this schemes will not affected any other schemes'); 
                                        }
                                    }
                                })
